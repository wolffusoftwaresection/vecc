...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<span class='btn btn-dropbox btn-flat selectFileBt'>选择文件</span>";
        btsStr += "</div>";
        btsStr += "<span class='btn btn-dropbox btn-flat uploadFileBt'  style='color:#FFF'>";
        btsStr += "<span class='iconfont icon-shangchuan'style='color:#FFF'></span>";
        btsStr += " </span>";
        btsStr += "<span class='btn btn-dropbox btn-flat cleanFileBt' style='color:#FFF;margin-left:15px;'>";
        btsStr += "<span class='iconfont icon-qingchu' style='color:#FFF'></span>";
        btsStr += " </span>";
        btsStr += "</div>";
    ...
    "addFileList":function(fileList,opt){
        ...
        if(fileNumber+fileList.length>opt.maxFileNumber){
            msg.error("最多只能上传" + opt.maxFileNumber + "个文件");
            return;
        }
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                msg.error("不支持该格式文件上传:" + fileList[i].name);
            }
        }
        ...
        if(uploadUrl!="#"&&uploadUrl!=""){
            ...
            $.ajax({
                ...
                success:function(data){
                    setTimeout(function(){opt.onUpload(data)},500);
                },
                error:function(e){
