//function 物流查询
function showexprecess(id) {
    var params = {};
    params.logisticsId = $("#" + id).val();
    ajax.get("Ajax/GetLogistics", params, function (e) {
...
function texteditbutton(id) {
	$("#" + id).summernote({
		...
		toolbar: [
		    ...
		    ['options', ['codeview']] //no help button

	]
	});
	var obj = $("#" + id).next().find("div");
	obj.find("button[data-toggle='dropdown']").click(function () {
...
//弹出daterangepicker
function showdaterangepicker(id) {
	$('#' + id).daterangepicker({
...
function showinputmask(id, regex) {

	$("#" + id).inputmask('Regex', { regex: regex });
}


function showuploadfile(id) {
	msg.load('Upload/File', '上传附件', function () {
		var iframeid = $("#uploadiframe").find("iframe").attr("id");
		var values = $("#" + iframeid)[0].contentWindow.getUrl();
		if (values[0]) { 
			var pdf = values[0];
			var pdfs = pdf.split(",");
			...
			for (var i = 0; i < pdfs.length; i++) {
				if (pdfs[i]) {
					str += '<div class="file-preview-frame krajee-default  kv-preview-thumb ">';
					str += '    <div class="kv-file-content" >';
					str += '        <div class="kv-preview-data file-preview-other-frame">';
					str += '            <div class="file-preview-other"><span class="file-other-icon"><i class="glyphicon glyphicon-file"></i></span></div>';
					str += '        </div>';
					str += '     </div>';
					str += '     <div class="file-thumbnail-footer">';
					str += '           <div class="file-footer-caption" title="">' + names[i] + '</div>';
					str += '                 <div class="file-actions">';
					str += '                       <div class="file-footer-buttons"> <button type="button" class="kv-file-remove btn btn-xs btn-default delpdf" id="delpdf_' + id + '" src="' + pdfs[i] + '" onclick="delpdf(this)" title="删除文件"><i class="glyphicon glyphicon-trash text-danger"></i></button>&nbsp;';
					str += '                       <button type="button" class="kv-file-zoom btn btn-xs btn-default" title="下载" id="prepdf_' + id + '" src="' + pdfs[i] + '" onclick="downfile(this)"><i class="fa fa-download"></i></button>     </div>';
					str += '            </div>';
			...
			$("#img_" + id).html($("#img_" + id).html() + str);
			$("#" + id).val($("#" + id).val() + pdfs);
		}
	})
	//openNewPage('Upload/index', '上传', 'btn_query_');
}
...
function showuploadpdf(id) { 
	msg.load('Upload/PDF', '上传文件', function () {
		var iframeid = $("#uploadiframe").find("iframe").attr("id");
		var values = $("#" + iframeid)[0].contentWindow.getUrl();
		if (values[0]) {
			...
			for (var i = 0; i < pdfs.length; i++) {
				if (pdfs[i]) {
					...
					str += '                 <div class="file-actions">';
					str += '                       <div class="file-footer-buttons"> <button type="button" class="kv-file-remove btn btn-xs btn-default delpdf" id="delpdf_' + id + '" src="' + pdfs[i] + '" onclick="delpdf(this)" title="删除文件"><i class="glyphicon glyphicon-trash text-danger"></i></button>&nbsp;';
					str += '                       <button type="button" class="kv-file-zoom btn btn-xs btn-default" title="查看详情" id="prepdf_' + id + '" src="' + pdfs[i] + '" onclick="prepdf(this)"><i class="glyphicon glyphicon-zoom-in"></i></button>     </div>';
					str += '            </div>';
					str += '            <div class="clearfix"></div>';
					str += '     </div>';
					str += '</div>';
...
function delpdf(obj) {
	var id = $(obj).attr("id").replace("delpdf_", "");
	var src = $(obj).attr("src");
	//
	$("#" + id).val($("#" + id).val().replace(src + ",", ""));
	//删除图片
...
function prepdf(obj) {
	...
	var src = $(obj).attr("src");
		...
		var w = $(document).width() > 800 ? 800 : $(document).width();
		//window.open("/scripts/plugins/pdfjs/web/viewer.html?FileUrl=" + encodeURIComponent(src));
		//return;
		layer.open({
			type: 2,
			title: 'PDF 浏览',
			...
			shadeClose: true,
			content: "/scripts/plugins/pdfjs/web/viewer.html?FileUrl=" + encodeURIComponent(src)
		});
...
function showuploadimg(id) {
	...
	msg.load('Upload/index?id=' + id, '上传图片', function () {
		 
		var iframeid = $("#uploadiframe").find("iframe").attr("id"); 
		var values = $("#" + iframeid)[0].contentWindow.getUrl();
		if (values) {
			
			var image = $("#" + id).val() + values;
			images = image.split(",");
			//
			var str = "";
			for (var i = 0; i < images.length; i++) {
				if (images[i]) {
					str += '<div class="file-preview-frame krajee-default  kv-preview-thumb"  data-fileindex="0" data-template="image">';
					str += '<div class="kv-file-content" ><img src="' + images[i] + '" class="file-preview-image kv-preview-data"  style="max-width:160px;height:160px;"/></div>';
					str += '<div class=""><div class="file-actions">';
					str += '<div class="file-footer-buttons"> <button type="button" class="kv-file-remove btn btn-xs btn-default delimg" id="delimg_'+id+'" onclick="delimg(this)" title="删除文件"><i class="glyphicon glyphicon-trash text-danger"></i></button>&nbsp;';
					str += '<button type="button" class="kv-file-zoom btn btn-xs btn-default" title="查看详情" id="preimg_' + id + '" onclick="preimg(this)"><i class="glyphicon glyphicon-zoom-in"></i></button>     </div>';
					str += '<div class="clearfix"></div>';
					str += '</div></div></div>';
				}
			}
...
function delimg(obj) {
	var id = $(obj).attr("id").replace("delimg_", "");
	var src = $(obj).parents('.file-preview-frame').find(".kv-file-content").find("img").attr("src");
	//
	$("#" + id).val($("#" + id).val().replace(src+",", ""));
	//删除图片
	$(obj).parents(".file-preview-frame").remove();
...
function preimg(obj) {
	...
	img.src = src; 
	var prew = img.width;
	if (prew > ($(document).width() - 10))
	prew = ($(document).width() - 10)
	layer.open({
		  type: 1,
		  title: false,
		  closeBtn: 0,
		  area:prew+ 'px',
		  //area: ($(document).width()-10)+'px',
		  skin: 'layui-layer-nobg', //没有背景色
		  shadeClose: true, 
		  content: '<img src="' + src + '"  style="max-width:' + ($(document).width() - 10) + 'px;" />'
	});
}


function toggleDiv(obj) {
	var id = $(obj).attr("id").replace("toggle_", "");
	$("#div_" + id).toggle();
}
...
function setPro(id) {
	...
	for (var i = 0; i < provinceList.length; i++) {
		...
		$("#" + id).append('<option value="' + provinceList[i].name + '">' + provinceList[i].name + '</option>');
...
function changepro(obj) {
	...
	children.each(function () {
		...
		var grandson = $("select[pid='" + cid + "']");
		grandson.html("");
		grandson.append('<option value=""></option>');
	})

	for (var i = 0; i < provinceList.length; i++) {
		if ($(obj).val() == provinceList[i].name) {
			item = provinceList[i];
		}
	}
	for (var i = 0; i < item.cityList.length; i++) {
		children.append('<option value="' + item.cityList[i].name + '">' + item.cityList[i].name + '</option>')
	}
}
function changecity(obj) {
	...
	for (var i = 0; i < provinceList.length; i++) {
		...
		if ($("#" + pid).val() == provinceList[i].name) {
			item = provinceList[i];
		}
	}
	var area;
	if (item) {
		for (var i = 0; i < item.cityList.length; i++) {
			if ($(obj).val() == item.cityList[i].name) {
				area = item.cityList[i];

			}
		}

		for (var i = 0; i < area.areaList.length; i++) {
			children.append('<option value="' + area.areaList[i] + '">' + area.areaList[i] + '</option>')
		}
	}
}
...
//页面控件
var _controllerHead = '<div class="col-sm-6" style="">   <div class="form-group">';
var _controllerFoot = '</div></div>'
function inputSelectAll(id, data, lang, name, value,type,languageId) { //控件样式
    
    switch(type)
    {
        case "DateTime": return inputSelectDateTimeStyle(id, data, lang, name, value, languageId);
            break;
        case "Delete": return inputSelectDisStyle(id, data, lang, name, value, languageId);
            break;
        case "Select": return inputSelectStyle(id, data, lang, name, value, languageId);
            break;
        case "Number": return inputSelectNumberStyle(id, data, lang, name, value, languageId);
            break
        case "Icon": return inputSelectIconStyle(id, data, lang, name, value, languageId);
            break;
        case "Remark": return inputSelectRemarkStyle(id, data, lang, name, value, languageId);
            break;
        default:
            return inputSelectOtherStyle(id, data, lang, name, value, languageId);
    }
    
}

function showAndbindInput(id,obj) {
    obj.sort(by("sort"));
    for (var i = 0; i < obj.length; i++) {
        $("#" + id).append(obj[i].html); 
        switch (obj[i].type) {
            case "DateTime": $('#' + obj[i].id).datepicker({ autoclose: true });
                break;
            case "Select": writeAdminOption(obj[i].url, obj[i].id, obj[i].value);
                break;
            default: "";
        }
    }
}
function inputSelectStyle(id, data, lang, languageId) {
    var _Controller = '';
    _Controller = _controllerHead; 
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">';
    _Controller += '          <select '  + '  id="' + id + '" name="' + id + '" placeholder="' + lang[0] + '" class="form-control search_' + id.split("_")[0] + '">';
    _Controller += '             <option value=""></option>';
    _Controller += '           </select>';
    _Controller += '     </div>';
    _Controller += _controllerFoot;
    return _Controller;
}
function inputSelectDisStyle(id, data, lang, name, value, languageId) {
    
    var _Controller = '';
    _Controller = _controllerHead;
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">';
    _Controller += '          <select ' + data[0] + data[1] + data[2] + 'placeholder="' + lang[0] + '"  id="' + id + '" name="' + id + '" class="form-control search_' + id.split("_")[0] + '">';
    _Controller += '             <option value=""></option>';
    if (0 == value) {
        _Controller += '              <option value="0" selected>' + lang[1] + '</option>';
        _Controller += '              <option value="1">' + lang[2] + '</option>';
    } else if (1 == value) {
        _Controller += '              <option value="0" >' + lang[1] + '</option>';
        _Controller += '              <option value="1" selected>' + lang[2] + '</option>';
    } else {
        _Controller += '              <option value="0" >' + lang[1] + '</option>';
        _Controller += '              <option value="1" >' + lang[2] + '</option>';
    }
...
function inputSelectDateTimeStyle(id, data, lang, name, value, languageId) {

    var _Controller = '';
    _Controller = _controllerHead;
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">';
    _Controller += '        <div class="input-group date">';
    _Controller += '          <input type="text" class="form-control pull-right search_' + id.split("_")[0] + '"  id="' + id + '" ' + data[0] + data[1] + data[2] + '  name="' + id + '" placeholder="' + lang[0] + '" value="' + value + '">';
    _Controller += '          <div class="input-group-addon">';
    _Controller += '                 <i class="fa fa-calendar"></i>';
    _Controller += '          </div>'; 
    _Controller += '        </div>';
    _Controller += '     </div>';
    _Controller += _controllerFoot;
...
function inputSelectNumberStyle(id, data, lang, name, value, languageId) {

    var _Controller = '';
    _Controller = _controllerHead;
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">';
    _Controller += '          <input type="number" class="form-control  search_' + id.split("_")[0] + '"  id="' + id + '" ' + data[0] + data[1] + data[2] + '  name="' + id + '" placeholder="' + lang[0] + '"  value="' + value + '">';
    _Controller += '      </div>';
    _Controller += _controllerFoot;
    return _Controller;
 
}
function inputSelectIconStyle(id, data, lang, name, value, languageId) {

    var _Controller = '';
    _Controller = _controllerHead;
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">';
    _Controller += '      <div class="input-group">   <input type="text" class="form-control search_' + id.split("_")[0] + '"  id="' + id + '" name="' + id + '" placeholder="' + lang[0] + '" value="' + value + '" readonly><div class="input-group-addon" onclick="showIcon(\''+ id +'\')" style="cursor:pointer"><i class="fa fa-search"></i> </div> </div>';
    _Controller += '      </div>';
    _Controller += _controllerFoot;
    return _Controller;

}
function inputSelectRemarkStyle(id, data, lang, name, value, languageId) {

    var _Controller = '';
    _Controller = '<div class="col-sm-12" style="">   <div class="form-group">';
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">';
    _Controller += '          <textarea class="form-control search _' + id.split("_")[0] + '" style="    height: 100px !important;"  id="' + id + '" name="' + id + '" placeholder="' + lang[0] + '" value="' + value + '"></textarea>';
    _Controller += '      </div>';
    _Controller += _controllerFoot;
    return _Controller;

}
function inputSelectOtherStyle(id, data, lang, name, value, languageId) {

    var _Controller = '';
    _Controller = _controllerHead;
    _Controller += '      <label for="' + id + '" class="col-md-4 control-label" data-language=' + languageId + '>' + lang[0] + '：</label>';
    _Controller += '      <div class="col-md-8">'; 
    _Controller += '          <input type="text" class="form-control  search_' + id.split("_")[0] + '"  id="' + id + '" ' + data[0] + data[1] + data[2] + '  name="' + id + '" placeholder="' + lang[0] + '" value="' + value + '">';
    _Controller += '      </div>';
    _Controller += _controllerFoot;
    return _Controller;                                         
...
function inputTableAll(id, name, show, type, key, controllernames,languageId) { //控件样式
 
    switch (type) {
        case "DateTime": return inputTableTimeStyle(id, name, show, languageId);
            break;
        case "Delete": return inputTableDeleteStyle(id, name, show, key, controllernames, languageId);
            break;
        
        default:
            return inputTableOtherStyle(id, name, show, languageId);
    }

}
function showAndbindtable(obj) { 
    obj.sort(by("sort"));
    var res=""
...
function inputTableTimeStyle(id, name, show, languageId) {
    
    return "{field: '" + id + "',align: 'center',title: '" + languagePlugin.GetPageLanguage(languagePlugin.AllLanguageUrl, languageId == "" ? '' : languageId, name) + "',visible: " + show + ",formatter: function (value, row, index) {return timeStamp2String(value);}},"
}
function inputTableDeleteStyle(id, name, show, key, controllernames, languageId) {
    var nodel = '<div class="switch"><div class="checkbox" style="     margin-top: 0px;  margin-bottom: 0px; "><input type="checkbox" onclick="closeOropenItemsCheck(this,\'' + controllernames[0] + '\')" ><label></label></div></div>';
    var yesdel = '<div class="switch"><div class="checkbox" style="     margin-top: 0px;  margin-bottom: 0px; "><input type="checkbox" onclick="closeOropenItemsCheck(this,\'' + controllernames[0] + '\')" ><label></label></div></div>';
    return "{field: '@" + id + "',align: 'center',title: '" + languagePlugin.GetPageLanguage(languagePlugin.AllLanguageUrl, languageId == "" ? '' : languageId, name) + "',visible: " + show + ",formatter: function (value, row, index) {value=(row.IsDeleted); var id= eval('row.'+'" + key + "');if (value == '1') { return  closeOropenItemsChoose(\''+id+'\',\'" + controllernames[0] + "\',\'" + controllernames[1] + "\',1,this); } else {return closeOropenItemsChoose(\''+id+'\',\'" + controllernames[0] + "\',\'" + controllernames[1] + "\',0,value,this);  }}},";

    return "";
       
    //return "{field: '" + id + "',align: 'center',title: '" + name + "',visible: " + show + ",formatter: function (value, row, index) {return timeStamp2String(value);}},"
}
function inputTableOtherStyle(id, name, show,languageId) {
    var title = languagePlugin.GetPageLanguage(languagePlugin.AllLanguageUrl, languageId == "" ? '' : languageId, name);
    return "{field: '" + id + "',align: 'center',title: '" + title + "',visible: " + show + "},"
}
